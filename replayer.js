...
(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        language: {
            offlineText: 'Temporalmente no disponible', // On offline: status bar text @string
            playedText: 'Recien sonaba', // Played title (player bottom) @string
            unknownTrackText: 'Cancion desconocida', // Set unknown track text when error
            unknownArtistText: 'Artista desconocido' // Set unknown artist text when error
    ...
    Plugin.prototype = {
        init: function () {
            ...
            // -- Status bar HTML
            if (ot.statusBar) {
                var muteUnmuteHtml = '<i id="kast-volume" class="kast-mdi kast-mdi-volume-off"></i><i id="kast-volume" class="kast-mdi kast-mdi-volume-high"></i>',
            ...
            if (ot.statusBar) {
                ...
                if (ot.muteUnmute) {
                    var up = document.getElementsByClassName('kast-mdi-volume-high');
                    up[0].onclick = function () {
                        if(that.audio.volume < 1) {
                            that.audio.volume += .25;
                        }
                    };
                     var down = document.getElementsByClassName('kast-mdi-volume-off');
